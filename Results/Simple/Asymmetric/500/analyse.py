	...
	for row in reader:
		...
		for x in range(0, 4):
			if len(row[x]) == 1:
				...
				name[x] = row[x]
		with open(name[0]+'-'+name[1]+'-'+name[2]+'-'+name[3]+'.csv') as testfile:
			...
			for row in newreader:
				...
				# 	total[third] += float(row[14])
				# 	if max(float(row[4]), float(row[9]), float(row[14])) == float(row[4]):
				# 		winner[first] += 1
				# 	elif max(float(row[4]), float(row[9]), float(row[14])) == float(row[9]):
				# 		winner[second] += 1
				# 	else:
				# 		winner[third] += 1
				...
				# 	total[second] += float(row[9])
				# 	total[third] += float(row[14])
				# 	total[forth] += float(row[19])
				# 	if max(float(row[4]), float(row[9]), float(row[14]), float(row[19])) == float(row[4]):
				# 		winner[first] += 1
				# 	elif max(float(row[4]), float(row[9]), float(row[14]), float(row[19])) == float(row[9]):
				# 		winner[second] += 1
				# 	elif max(float(row[4]), float(row[9]), float(row[14]), float(row[19])) == float(row[14]):
				# 		winner[third] += 1
				# 	else:
				# 		winner[forth] += 1
...
# 			else:
# 				name[x] = row[x]
# 		with open('Results/BSE/bse-'+name[0]+'-'+name[1]+'-'+name[2]+'-'+name[3]+'.csv') as testfile:
# 			newreader = csv.reader(testfile, delimiter=',')
# 			total = [0, 0, 0, 0]
# 			winner = [0, 0, 0, 0]
# 			if int(name[3]) != 0:
